...
import { useGetPrimaryImageByModelNameQuery } from "@/redux-store/services/BikeSystemApi/bikeImageApi";
...
export function CustomerBikeInfo() {
  ...
  // Derive modelName before early returns — hook must be called unconditionally
  const firstVehicleModelName = vehiclesData?.data?.[0]?.modelName ?? "";
  ...
  const vehicleImageAlt = bikeImageData?.data?.alt ?? firstVehicleModelName;
