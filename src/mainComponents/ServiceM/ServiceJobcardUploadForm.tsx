...
interface ServiceJobcardUploadFormProps {
  dashboardPath?: string;
}

/**
 * Single-shot service-jobcard upload (no preview/column-selection step,
 * matching /api/service-jobcard/import's contract): parse+diff+persist+
 * auto-register happen in one request. Result is either "no changes
 * detected" (duplicate) or an added/changed/revenue-delta summary with the
 * rendered changelog and the customer/vehicle auto-registration summary.
 */
export default function ServiceJobcardUploadForm({
  dashboardPath = "/service-admin/dashboard",
}: ServiceJobcardUploadFormProps) {
  const navigate = useNavigate();
  const inputRef = useRef<HTMLInputElement>(null);
  const [importServiceJobcardReport, { isLoading }] =
    useImportServiceJobcardReportMutation();

  const [file, setFile] = useState<File | null>(null);
  const [validationError, setValidationError] = useState<string | null>(null);
  const [dragOver, setDragOver] = useState(false);
  const [result, setResult] = useState<ServiceJobcardImportResponse["data"] | null>(
    null,
  );
  ...
  return (
    <div className='max-w-3xl mx-auto p-6 space-y-6'>
      <Card className='border border-gray-200 shadow-sm'>
        <CardHeader>
          <CardTitle>Upload Service Job Card Report</CardTitle>
          <p className='text-sm text-gray-500'>
            Compares this file against your last upload — only new or
            corrected job cards are imported.
          </p>
        </CardHeader>
            ...
            {result.autoRegistration && (
              <div className='rounded-lg border border-gray-200 p-4'>
                ...
                <div className='grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm'>
                  ...
                  <div>
                    <p className='text-xs text-gray-500'>Name mismatches flagged</p>
                    <p className='font-semibold text-amber-700'>
                      {result.autoRegistration.nameMismatches}
                    </p>
                  </div>
                </div>
              </div>
